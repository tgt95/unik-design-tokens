...
function makeColorRow(raw, res) {
  ...
  row.appendChild(makeText(isRGBA(res) ? rgbaToHex(res) : '—', { fontSize: 11, opacity: 0.9 }))
  return row
}
...
async function buildTable(selected, u) {
  ...
  for (var cix = 0; cix < collections.length; cix++) {
    ...
    for (var k = 0; k < varsInCollection.length; k++) {
      ...
      var row = makeRow(
        [makeCell(typeLabel, 80), makeCell(tokenName, 280)].concat(modeCells).concat([makeCell(desc, 360)])
      )
