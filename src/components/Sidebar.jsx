...
export default function Sidebar({ items, name = 'Table of contents' }) {
  ...
  return (
    <nav className='sidebar'>
      ...
      <ul>
        {items.map((node) => (
          <SidebarNode key={node.id} node={node} depth={0} prefix={node.id} onItemClick={handleClick} />
        ))}
      </ul>
    </nav>
  )
      ...
      {node.children.length > 0 && (
        <ul>
          {node.children.map((child) => (
            <SidebarNode key={child.id} node={child} depth={depth + 1} prefix={prefix} onItemClick={onItemClick} />
          ))}
        </ul>
      )}
    </li>
