...
export default function App() {
  ...
  useEffect(() => {
    async function loadAll() {
      ...
      const globalData = await Promise.all(allFiles.map((f) => fetch(assetUrl(`/tokens/${f}`)).then((r) => r.json())))
      ...
      for (const [collectionName, collection] of Object.entries(manifest.collections)) {
        ...
        for (const [modeName, files] of Object.entries(collection.modes)) {
          const localData = await Promise.all(files.map((f) => fetch(assetUrl(`/tokens/${f}`)).then((r) => r.json())))
